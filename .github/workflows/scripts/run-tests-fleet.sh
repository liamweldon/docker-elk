...
for _ in $(seq 1 30); do
	response="$(curl 'http://elasticsearch:9201/metrics-system.cpu-default/_search?q=agent.name:%22fleet-server%22%20AND%20agent.type:%22metricbeat%22%20AND%20event.module:%22system%22%20AND%20event.dataset:%22system.cpu%22%20AND%20metricset.name:%22cpu%22&size=1&pretty' -s --resolve "elasticsearch:9201:${ip_es}" -u elastic:testpasswd)"
...
# retry for max 60s (30*2s)
for _ in $(seq 1 30); do
	response="$(curl 'http://elasticsearch:9201/logs-docker.container_logs-default/_search?q=agent.name:%22fleet-server%22%20AND%20agent.type:%22filebeat%22%20AND%20container.name:%22docker-elk-elasticsearch-1%22&size=1&pretty' -s --resolve "elasticsearch:9201:${ip_es}" -u elastic:testpasswd)"
