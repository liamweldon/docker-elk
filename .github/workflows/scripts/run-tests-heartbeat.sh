...
for _ in $(seq 1 30); do
	response="$(curl 'http://elasticsearch:9201/heartbeat-*/_search?q=agent.type:%22heartbeat%22%20AND%20monitor.type:%22http%22%20AND%20url.domain:%22elasticsearch%22&size=1&pretty' -s --resolve "elasticsearch:9201:${ip_es}" -u elastic:testpasswd)"
